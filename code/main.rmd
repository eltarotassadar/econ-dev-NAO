...
## Читаем данные с термокарстом (которые до этого были преобразованы в Jupyter)
...
## Создаем стак с нужными переменными
...
## Форматируем данные под модель: добавляем существующие пикселы с координатами, и рандомно добавляем отсутствующие
...
## Настройка индивидуальных параметров модели
...
## Запускаем индивидуальные встроенные модели
...
##Важность переменных (!можно использовать все, что в открытом доступе, и доп оценить антроп)

```{r}
(termo_models_1_var_import <- get_variables_importance(termo_models_1))
```

## Усредним "важность"за все прогоны + FULL

```{r}
apply(termo_models_1_var_import, c(1,2), mean)
```

## Кривые отлкика для каждой модели
...
##по X - значение переменной, по Y -  отклик 
```

## Ансамблевые модели (т.е. одновременное обучение моделей)
...
termo_ensemble_models_1 <- 
  BIOMOD_EnsembleModeling(
    ...
    em.by = 'all',
    eval.metric = 'TSS',
    eval.metric.quality.threshold = 0.6, #выбираем модели, у которых такие метрики
...
##Считаем SRC (species range change)
##подгрузка проекций
```{r}
